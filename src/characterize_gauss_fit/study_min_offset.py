...
def run(cfg: Config, *, num_workers: int = 1) -> None:
    ...
    _write_outputs(cfg, specs, results, study_dir=study_dir)
...
def _write_outputs(
    ...
    *,
    ...
    Raises:
        RuntimeError: If the number of result buckets consumed during bucketing
            does not match the total number of results returned by
            :func:`~executor.run_trials`, indicating a mismatch between the
            specs generated by :func:`build_specs` and the results list.
    """
    study = cfg.studies.min_detectable_offset
    scale = cfg.generation.scale
