...
def test_gaussian_1d():
    assert GaussianPSF.gaussian_1d(0.0) == pytest.approx(0.39894228)
    assert GaussianPSF.gaussian_1d(0.0, scale=2.0) == pytest.approx(0.39894228 * 2)
    assert GaussianPSF.gaussian_1d(0.0, base=-5.0) == pytest.approx(0.39894228 - 5.0)
    assert GaussianPSF.gaussian_1d(0.0, scale=2.0, base=-5.0) == pytest.approx(0.39894228 * 2 - 5.0)
    assert GaussianPSF.gaussian_1d(1.0, mean=1.0) == pytest.approx(0.39894228)
    assert GaussianPSF.gaussian_1d(1.0, mean=1.0, scale=2.0) == pytest.approx(0.39894228 * 2)
    assert GaussianPSF.gaussian_1d(1.0) == pytest.approx(0.24197072)
    assert GaussianPSF.gaussian_1d(2.0, sigma=2.0) == pytest.approx(0.24197072 / 2)
...
def test_gaussian_2d():
    ...
    assert GaussianPSF.gaussian_2d(0, 0, scale=2.0) == pytest.approx(0.15915494309 * 2)
    assert GaussianPSF.gaussian_2d(0, 0, base=10.0) == pytest.approx(0.15915494309 + 10)
    assert GaussianPSF.gaussian_2d(0, 0, scale=2.0, base=10.0) == pytest.approx(
        0.15915494309 * 2 + 10
    )
    assert GaussianPSF.gaussian_2d(1.0, 0.0, mean_y=1.0) == pytest.approx(0.15915494309)
    assert GaussianPSF.gaussian_2d(0.0, 1.0, mean_x=1.0) == pytest.approx(0.15915494309)
    ...
    assert GaussianPSF.gaussian_2d(1.0, 0.0) == pytest.approx(0.09653235263005391)
...
def test_gaussian_eval_point():
    ...
    assert psf1.eval_point((2, 3), sigma=(1.0, 2.0)) == psf4.eval_point((2, 3))
    ...
    assert psf1.eval_point(
        (2, 3), sigma=(1.0, 2.0), base=1, scale=2, angle=np.pi / 4
    ...
    ret = psf1.eval_point((np.array([1, 2]), np.array([2, 3])), sigma=(1.0, 2.0))
...
def test_gaussian_eval_pixel():
    ...
    assert GaussianPSF().eval_pixel((0, 0), sigma=2.0) == pytest.approx(integ2)
...
def test_gaussian_eval_rect():
    ...
    assert np.sum(GaussianPSF(sigma=(1, 1), angle=np.pi / 4).eval_rect((19, 19))) == pytest.approx(
        1
    )
...
def test_gaussian_find_position(use_angular_params, bkgnd_degree):
    ...
    psf = GaussianPSF()
    gauss2d = psf.eval_rect((21, 21), scale=2.0, sigma=1.0)
    # if allow_nonzero_base:
    #     psf = GaussianPSF(detailed_logging=True)
